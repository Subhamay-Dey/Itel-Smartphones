...
const FeatureRich = ({ title, description, cards = [] }) => {
    return (
        <div className='content overflow-hidden py-8 md:py-[120px] space-y-6 md:space-y-24 font-[MARK_OT] px-4 md:px-0'>
            <div className="flex flex-col lg:flex-row gap-3 justify-between">
                <h1 className="lg:text-desktop/h2 text-mobile/h4 text-itel-red max-w-[450px] font-[MARK_OT]" data-aos="fade-up">{title}</h1>
            ...
            <div className="flex flex-col gap-6 text-dark/card/bg justify-between lg:flex-row ">
                <For each={cards} render={({ title, description, icon, id }) => (
                    <div className="w-full max-w-[300px] space-y-2 md:space-y-4" key={id} data-aos="fade-up" data-aos-delay={id * 100}>
                        ...
                        <h2 className='text-mobile/h5/medium lg:text-desktop/h4'>{title}</h2>
                        <h5 className='text-desktop/body/2/regular text-brand/black/70 lg:text-desktop/body/1'>{description}</h5>
                    </div>
                )} />
            </div>
        </div>
